...
def create_plugin_entry(name, repo, plugin_type="start", tag=None, branch=None):
    entry = {
        "name": name,
        "repo": repo,
        "type": plugin_type
...
def add_plugin(identifier, tag=None, branch=None, plugin_type="start"):
    try:
    ...
    if plugin_exists_in_json(plugins, name):
        print(f"Plugin '{name}' already exists in plugins.json")
        return

    new_plugin = create_plugin_entry(name, repo, plugin_type, tag, branch)
    plugins.append(new_plugin)
    save_plugins(plugins)

    print(f"Added plugin '{name}' ({repo}) to plugins.json")
    print("Do not run clean before installing or this addition will be removed")
